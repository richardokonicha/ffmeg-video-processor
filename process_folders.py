        ...
        for lang_dir in audio_dir.iterdir():
            if lang_dir.is_dir():
                ...
                with open(caption_file_path, "r") as caption_file:
                    caption_lines = re.findall(pattern=r'\d+\n(.+)', string=caption_file.read())

                # Check for audio files and corresponding image files
                for i, (audio_file, caption_text) in enumerate(zip(audio_files, caption_lines)):
                    ...
                    subprocess.run(["ffmpeg", "-hide_banner", "-y", "-i", str(image_file), "-i", str(audio_file),
                                    ...
                                    f"[vout]drawtext=text='{caption_text}':fontcolor=white:fontsize=24:"
                ...
                subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", "list.txt", "-c", "copy", str(video_file)])
